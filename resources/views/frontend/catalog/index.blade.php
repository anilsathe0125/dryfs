                ...
                <div class="shop-top-filter-wrapper">
                    <div class="row">
                        ...
                        <div class="col-md-2 gd-text-sm-center">
                            <div class="shop-view"><a class="list-view {{ Session::has('view_catalog') && Session::get('view_catalog') == 'grid' ? 'active' : '' }} " data-step="grid" href="javascript:;" data-href="{{ route('frontend.catalog') . '?view_check=grid' }}"><i class="fas fa-th-large"></i></a>
                                <a class="list-view {{ Session::has('view_catalog') && Session::get('view_catalog') == 'list' ? 'active' : '' }}" href="javascript:;" data-step="list" data-href="{{ route('frontend.catalog') . '?view_check=list' }}"><i class="fas fa-list"></i></a>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-3 order-lg-1">
                ...
                <aside class="sidebar sidebar-offcanvas position-left"><span class="sidebar-close"><i
                    ...
                    @foreach ($attributes as $attribute)
                        <section class="widget widget-categories card rounded p-4">
                            <h3 class="widget-title">{{ __('Filter by') }} {{ $attribute->name }}</h3>
                            @foreach ($attribute->options as $options)
                                <div class="custom-control custom-checkbox">
                                    <input class="custom-control-input option"
                                        {{ isset($subcategory) && $subcategory->id == $options->id ? 'checked' : '' }}
                                        type="checkbox" value="{{ $options->name }}"
                                        id="{{ $attribute->id }}{{ $options->name }}">
                                    <label class="custom-control-label"
                                        for="{{ $attribute->id }}{{ $options->name }}">{{ $options->name }}<span
