        ...
        <div class="row">
            ...
            <div class="col-xl-9 col-lg-8">
                <div class="steps flex-sm-nowrap mb-5">
                    <a class="step" href="{{ route('frontend.checkout.billing') }}">
                        <h4 class="step-title">1. {{ __('Billing Address') }}:</h4>
                    </a>
                    <a class="step active" href="{{ route('frontend.checkout.shipping') }}">
                        <h4 class="step-title">2. {{ __('Shipping Address') }}:</h4>
                    </a>
                    <a class="step" href="{{ route('frontend.checkout.payment') }}">
                    ...
                    <div class="card-body">
                        ...
                        <form id="checkoutShipping" action="{{ route('frontend.checkout.shipping.store') }}" method="POST">
                            ...
                            <div class="d-flex justify-content-between paddin-top-1x mt-4">
                                <a class="btn btn-outline-secondary btn-sm" href="{{ route('frontend.cart') }}"><i class="icon-arrow-left"></i><span class="hidden-xs-down">{{ __('Back To Cart') }}</span></a>
                                <button class="btn btn-primary  btn-sm" type="submit"><span class="hidden-xs-down">{{ __('Continue') }}</span><i class="icon-arrow-right"></i></button></div>
                        </form>
                    </div>
                </div>
            </div>
            <!-- Sidebar          -->
            @include('frontend._inc.checkout_sidebar', $cart)
        </div>
    </div>
@endsection
